@echo off
rem ----------------------------------------------------------------------------
rem  MM3DRead v0.1 * Status reader program for MM3D device
...
echo.
%PPC% %FPFLAG1% %FPFLAG2% %FPFLAG3% %FPFLAG4% mm3dread.lpr
echo.
...
if not exist "source\lib\%ARCH%-%OS%\mm3dread.exe" ( echo Error: firstly run "build.bat" to compile source code! & goto end )
...
config\mkshortcut.vbs /target:"%INSTDIR%\%NAME%.exe" /shortcut:"%USERPROFILE%\Desktop\MM3DRead"
echo.
...
rd /s "%INSTDIR%"
del /q "%USERPROFILE%\Desktop\MM3DRead.lnk"
goto end
